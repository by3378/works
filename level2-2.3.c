...
int main(void)
{
	...
	struct CHA_NNEL SELLER[BUYMAX];//定义结构数组SELLER,其中每个元素代表一次货物购买,最多允许顾客挑选10次(应该够了吧)
    for(int i=0;i<BUYMAX;i++)
    ...
    for(int i=0;i<CHANNELMAX;)
    {
        while(1)
    	{
						...
						&& (ptr[i].number<=NUMBERMAX && ptr[i].number>0)     )
        	{
				...
				if(i<CHANNELMAX)	
				    break;	//未到5，继续读取输入，达到5，跳过接下的else
        	}
        	else //超出范围或输入格式错误
        	{
    	    	puts("您的输入不合要求，请重新输入：");   
	        ...
	        while(i==5)
	        {
	        	puts("现在五个通道中都有货物，您只能输入BACK或END。");
	        	s_gets(temp,20);       
	    	
			    if ( strcmp(TOUPPER(temp),BACK)==0  )  //输入BACK，回退次数必然小于3，且必然不是退无可退   
	            {
	        	    count++;  
	        	    ptr[--i] = (struct cha_nnel){'#',0,0,0};//重置上次输入,防止用户直接输入END       
	        	    puts("已回退至上一步。");
	            ...
	            else if ( strcmp(TOUPPER(temp),END)==0 )
	            {
	        	    puts("您已完成货物摆放。");
	         	    return 0;
	        	} 
...
int goods_output_all(const struct cha_nnel ptr[])   //展示所有的货物
{
	...
	for(int p=1;p<=CHANNELMAX;p++)
	{
	    for(int i=0;i<CHANNELMAX;i++)
	    {
			if( ptr[i].channel==p && ptr[i].type!='#' && ptr[i].number!=0)//找到p通道所对应的序A号i
...
int goods_buy(struct CHA_NNEL PTR[],struct cha_nnel ptr[],int * total_money)//货物购买部分   
{
	...
	int count=0;   //回退次数
    int p;         //seller编号(摆放)
	for(int i=0;i<BUYMAX;)//SELLER编号（购买）
    {
        while(1)
    	{
    		            ...
    		            && (PTR[i].NUMBER<=NUMBERMAX && PTR[i].NUMBER>0)    )//数据格式正确
        	{
				...
				if(p>CHANNELMAX-1 || ( p<CHANNELMAX && ptr[p].type!=PTR[i].TYPE ))   //没找到对应的p或找到了但货物不对
		        {
...
void input_and_change_function(int total_money)//投币与找零
{
	...
	int input_money[50];//存放投币结果,最多允许用户投币50次（应该够了吧）
	...
	while(current_money<total_money)
    {
	    ...
	    {
	        count++;    
	        current_money-=input_money[--i];   //还原金额A
